...
export class LocaleService {
  ...
  async enabledLocalesMap() {
    const enabledLocales: any = await this.getEnables({
      search: '',
      pageSize: 10,
      page: 1,
      sortField: 'code',
  ...
  async getEnables(paginationParams?: PaginationDTO) {
    ...
    return result.data.map((item) => {
      return {
        ...
        name: (item as any).name || (item as any).code,
      }
    });
  }
  ...
  async listModelWithLocale(
    ...
    try {
      const fields = this.prismaService.getFields(modelName, true);
      let OR: any[] = [];
