...
const SIDEBAR_SCROLL_STORAGE_KEY = 'hedhog:nav:scrollTop';
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile}>
        <ResizableSheetContent
          sheetId="app-mobile-sidebar-sheet"
          defaultWidth={320}
          minWidth={280}
          maxWidth={480}
          enableResize={false}
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-card text-sidebar-foreground p-0"
          style={
            {
...
function SidebarTrigger({
  ...
  const isOpen = isMobile ? openMobile : false; // morph only on mobile
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={cn(
            'absolute h-[1.5px] w-4 rounded-full bg-current transition-all duration-300 ease-in-out motion-reduce:transition-none',
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'hover:after:bg-background-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 sm:flex',
...
function SidebarFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn('flex shrink-0 flex-col gap-2 border-t p-2', className)}
      {...props}
    />
  );
}
...
const SidebarContent = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<'div'>
>(({ className, onScroll, ...props }, forwardedRef) => {
  ...
  const restoreScrollPosition = React.useCallback(() => {
    if (typeof window === 'undefined' || isMobile || state === 'collapsed') {
      return;
    }

    const container = localRef.current;

    /* v8 ignore next 3 -- unreachable: the ref is always populated by setRefs before this effect-driven callback runs */
    if (!container) {
      return;
    }
