...
type Data = { kpis?: { approvalRate?: DashboardMetric } };

export default function ApprovalRateKpi({ widget, onRemove }: Props) {
  ...
  const { data, isLoading, isAccessDenied, isError } = useWidgetData<Data>({
    ...
    select: (d) => ({ kpis: { approvalRate: (d as any)?.kpis?.approvalRate } }),
  });

  const metric = data?.kpis?.approvalRate;
  ...
  return (
    <WidgetWrapper
      ...
      isError={isError}
      widgetName={widget?.name || t('kpis.approvalRate.label')}
      onRemove={onRemove}
    >
        ...
        <CardContent className="flex h-full flex-col justify-between p-5">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-1 text-xs">
              ...
              <span className="text-muted-foreground">{t('kpis.approvalRate.description')}</span>
            </div>
          </div>
        </CardContent>
      </Card>
