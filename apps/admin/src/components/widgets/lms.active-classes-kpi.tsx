...
type Data = { kpis?: { activeClasses?: DashboardMetric } };

export default function ActiveClassesKpi({ widget, onRemove }: Props) {
  ...
  const { data, isLoading, isAccessDenied, isError } = useWidgetData<Data>({
    ...
    select: (d) => ({ kpis: { activeClasses: (d as any)?.kpis?.activeClasses } }),
  });

  const metric = data?.kpis?.activeClasses;
  ...
  return (
    <WidgetWrapper
      ...
      isError={isError}
      widgetName={widget?.name || t('kpis.activeClasses.label')}
      onRemove={onRemove}
    >
        ...
        <CardContent className="flex h-full flex-col justify-between p-5">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-1 text-xs">
              ...
              <span className="text-muted-foreground">{t('kpis.activeClasses.description')}</span>
            </div>
          </div>
        </CardContent>
      </Card>
