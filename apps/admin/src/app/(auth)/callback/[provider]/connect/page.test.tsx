...
describe('Página de callback OAuth — connect', () => {
  beforeEach(() => {
    paramsState.provider = 'google';
    searchParamsBox.current = new URLSearchParams('code=abc123');
  ...
  it('renderiza o estado de carregamento com o título de conexão', async () => {
    setSearchParams('code=abc123');
    appState.request.mockResolvedValue({ data: {} });
    render(<Page />);
    expect(screen.getByText('connectTitle')).toBeInTheDocument();
  ...
  it('conecta com sucesso: define accessToken, mostra toast e navega para a lista de contas', async () => {
    setSearchParams('code=abc123');
    ...
    expect(router.push).toHaveBeenCalledWith('/core/account/accounts');
  });
  ...
  it('exibe erro e permite retry (navega para a lista de contas) quando o request falha', async () => {
    setSearchParams('code=abc123');
  ...
  it('exibe a mensagem "já processado" quando este código já foi tratado nesta sessão', async () => {
    ...
    window.sessionStorage.setItem('oauth-callback:google:connect:dup-code', 'processed');
    render(<Page />);
